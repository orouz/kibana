  ...
  describe('Findings Page', () => {
    ...
    describe('GroupBy', () => {
      it('groups findings by resource', async () => {
        ...
        expect(await findingsByResourceTable.getRowsCount()).greaterThan(0);
      });

      it('navigates to resource findings page from resource id link', async () => {
        await findingsByResourceTable.clickResourceIdLink(
          data[0].resource.id,
          data[0].rule.section
        );
        expect(await resourceFindingsTable.getRowsCount()).greaterThan(0);
      });
    });
  });
}
